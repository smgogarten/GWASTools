...
\Rcode{snpgdsBED2GDS} in the \Rcode{SNPRelate} package converts
binary PLINK to GDS.
...
converted into A allele dosage and stored in NetCDF with the functions
\Rcode{ncdfImputedDosage} or in GDS with \Rcode{gdsImputedDosage} .
