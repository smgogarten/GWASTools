...
or the Network Common Data Format (NetCDF) (\href{http://www.unidata.ucar.edu/software/netcdf/}).
...
%% to a standard set of variables defined for GWAS data.
...
and \Rcode{NcdfIntensityReader}, which are built on top of the \Rpackage{ncdf} package.
...
We create a NetCDF file from a set of plain text files containing the
genotypes, one file per sample. The data are written to the NetCDF file one sample at a time and, simultaneously, the corresponding sample identifier \Rcode{scanID} is written to the \Rcode{sampleID} variable. The \Rcode{file}
...
The function \Rcode{createDataFile} creates the two dimensions and
