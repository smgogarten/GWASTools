test_GdsGenotypeReader <- function() {  
  ...
  # check exception with incorrect genotypeDim
  checkException(GdsGenotypeReader(file, genotypeDim="notavalidstring"))
  checkException(GdsGenotypeReader(file, genotypeDim="scan,snp"))
  ...
  ## same thing but for scan,snp dimensions (transposed) ##
  file <- tempfile()
  gds <- createfn.gds(file)
  snp <- 1:260
  chrom <- rep(1:26, each=10)
  ...
  nsnp <- length(snp)
  nsamp <- length(samp)
  geno <- t(matrix(sample(0:3, nsnp*nsamp, replace=TRUE),
                 nrow=nsnp, ncol=nsamp))
  add.gdsn(gds, "snp.id", snp)
  add.gdsn(gds, "snp.chromosome", chrom)
  add.gdsn(gds, "snp.position", pos)
  ...
  checkIdentical(samp, getScanID(obj))
  geno[geno == 3] <- NA
  checkIdentical(t(geno), getGenotype(obj))
  ...
  # test that an exception is raised for a gds file with the same snp,scan dimensions if genotypeDim is not specified
  file <- tempfile()
  gds <- createfn.gds(file)
  snp <- 1:11
  chrom <- rep(1:11)
  ...
  # equal dimensions when genotypeDim is not set
  checkException(GdsGenotypeReader(file))
  # works when setting genotypeDim
  obj <- GdsGenotypeReader(file, genotypeDim="scan,snp")
  geno[geno < 0 | geno > 2] <- NA
  checkIdentical(t(geno), getGenotype(obj)) # TRUE
  close(obj)
  unlink(file)
