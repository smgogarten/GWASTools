...
path <- system.file("extdata", "illumina_raw_data",
  package="GWASdata")
@ 
...
the BAlleleFrequency and LogRRatio values are written to this NetCDF in a two dimensional format, 
with SNPs corresponding to rows and samples corresponding to columns.

\subsubsection*{Creating the BAlleleFrequency and LogRRatio NetCDF file}
...
We next call the {\tt ncdfAddData} function to populate the intensity NetCDF with the 
BAlleleFrequency and LogRRatio values. The sample identifier {\tt scanID} is also populated at the same time.
...
ncfile <- system.file("extdata", "illumina_geno.nc",
  package="GWASdata")
...
We will create some plots of the missing call rates so we can easily identify any outliers.
...
This step calculates {\tt missing.n2}, which is the missing call rate per SNPs with {\tt missing.e1} less 
...
We will create a histogram of the overall missing call rate per sample in order to identify any samples with
a relatively larger missing call rate.  Finally, we plot the samples sorted by missing call rate.
...
Next, identify windows within sample-chromosome pairs that have very high BAlleleFreq
...
Call {\tt chromIntensityPlot} to plot the BAlleleFreq of all SNPs on the indicated chromosome-sample
...
Now we will use the calculated mean intensities by sample to identify any gender mis-annotation or sex 
...
We assume the sample annotation file has the {\tt duplicated} variable, while the SNP file needs to have 
{\tt chromosome} and {\tt missing.n1} variables. We will convert the genotype NetCDF file to the {\tt gds} format with the function {\tt convertNcdfGds}.
...
ibdobj <- snpgdsIBDMLE(gdsobj, sample.id=sample.sel,
                       snp.id=snp.sel, method="EM")
closefn.gds(gdsobj)
names(ibdobj)
dim(ibdobj$k0)
ibdobj$k0[1:5,1:5]
...
relationship.
We can also assigned observed relationships based on the values of $k0$
and $k1$.
...
gdsobj <- openfn.gds(gdsfile)
ibdobj <- snpgdsIBDMoM(gdsobj, sample.id=sample.sel,
                    snp.id=snp.sel)
closefn.gds(gdsobj)
dim(ibdobj$k0)
ibdobj$k0[1:5,1:5]
...
Plot the principal component pairs for the first three PCs, by
case-control status. 
...
Do boxplots for the first few PCs to show differences between cases and controls, along with
...
We use circular binary segmentation to find change points in B Allele
Frequency (BAF).
...
legend("bottomright", unique(dat$pop), 
...
To further investigate SNPs with a high Mendelian error rate, we will make a cluster plot for 9 SNPs
with the highest Mendelian error rate. We expect the 
plots to lack defined genotype clusters, leading to a poor call rate. For this example, we will plot the top 9 SNPs  
using the function {\tt genoClusterPlot}.
...
@ 
 ...
 We will examine the CEU population data here; the remaining population groups and their results are analyzed in exactly the same manner.
...
To run the association test, we call the function {\tt
...
To create QQ plots of the ordered p-values from the association tests versus the ordered expected p-values, we first load the saved p-values data.
