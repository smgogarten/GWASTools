...
In the \Rpackage{GWASTools} package, access to the NetCDF files is provided by
the \Rcode{NcdfGenotypeReader} and \Rcode{NcdfIntensityReader} classes.
These classes are built on top of the \Rpackage{ncdf} package and provide access
...
slots for a \Rcode{NcdfGenotypeReader} (or \Rcode{NcdfIntensityReader})
...
\begin{itemize}
\item{\Rcode{scanID}, a unique integer identifier for each scan}.
...
The \Rpackage{GWASTools} package depends on the \Rpackage{ncdf} library, which provides a convenient R interface to create, populate and extract data from NetCDF files. 

For each study, three different NetCDF files are  
created to be used in subsequent cleaning and analysis steps.   
All NetCDF files created have two dimensions, one called \Rcode{snp} and one titled \Rcode{sample}.  The \Rcode{snp}
...
\Rcode{sample} dimension is of length equal to the number of genotyping scans released as listed in the sample annotation file.
...
(genotype calls, for example).  The \Rcode{sampleID} ordering must match the \Rcode{scanID} values as listed
in the sample annotation object (Section~\ref{create_sampann}).
The index to the SNP probes in the NetCDF file is the \Rcode{snpID}, which is stored as values of the SNP dimension.  Since \Rcode{snpID} is in chromosome and position order, these variables also provide a check on ordering and are often used to select subsets of SNPs for analysis.  Analogous to the sample ordering, these values must match the \Rcode{snpID} values listed
...
We recommend always checking the ordering of these variables before saving new versions of the SNP or sample annotation.
...
data will be written.  The function \Rcode{ncdfCreate} creates the two dimensions and 
three common variables as described above. It also assigns values of the SNP dimension as \Rcode{snpID} values and populates the \Rcode{chromosome} and \Rcode{position} variables.

In this case, we also want the \Rcode{genotype} variable to be created, so the \Rcode{vars} argument must be set to 
\Rcode{"genotype"}. In the interest of computational feasibility, only 3 samples will be written in this step.
...
@ 
 ...
 Now the shell has been created so we can call the \Rcode{ncdfAddData} function to 
populate the NetCDF with the genotype data stored in the raw data files.  The data are written to the NetCDF file one sample at a time and, simultaneously, the corresponding sample identifier \Rcode{scanID} is written to the \Rcode{sampleID} variable. The \Rcode{file}
...
The arguments to \Rcode{ncdfAddData} must be created. We need
      \Rcode{col.nums}, which is an integer vector indicating which columns
...
All required arguments have been defined so we are ready to call \Rcode{ncdfAddData}.
This function will take the previously created genotype NetCDF file and populate it with the genotype
data from the raw text files and with the sample identifier \Rcode{scanID} corresponding to that file in the sample annotation data.frame.
...
Run the function \Rcode{ncdfCheckGenotype} to check that the
...
\subsubsection*{Reading the Genotype NetCDF file}
          
The \Rcode{NcdfGenotypeReader} class provides a convenient interface for
    retrieving data from a genotype NetCDF file.  Some of the same
    ``get'' methods that applied to SNP and scan annotation data
    objects can be used for \Rcode{NcdfGenotypeReader} objects.

<<>>=
(nc <- NcdfGenotypeReader(geno.nc.file))
nscan(nc)
nsnp(nc)
head(getScanID(nc))
head(getSnpID(nc))
head(getChromosome(nc))
head(getPosition(nc))
# genotypes for the first 3 samples and  the first 5 SNPs
getGenotype(nc, snp=c(1,5), scan=c(1,3))
close(nc)
@ 
...
Aside from the three variables held in common with all NetCDF files (\Rcode{sampleID, chromosome} and \Rcode{position}),
...
genotype NetCDF, \Rcode{ncdfCreate}.  The two dimensions and three common variables will be created
along with the X, Y and quality variables; the \Rcode{vars} argument is set to write these.
...
We next call the \Rcode{ncdfAddData} function to populate the intensity NetCDF with the 
X and Y intensities and the quality score. The sample identifier \Rcode{scanID} is also populated at the same time.
...
Run the function \Rcode{ncdfCheckIntensity} to check that the
    NetCDF file contains the same data as the raw data files.
...
\subsubsection*{Reading the Intensity NetCDF file}
          
The \Rcode{NcdfIntensityReader} class provides a convenient interface for
    retrieving data from an intensity NetCDF file.  Its methods are
    similar to \Rcode{NcdfGenotypeReader}.

<<>>=
(nc <- NcdfIntensityReader(qxy.nc.file))
nscan(nc)
nsnp(nc)
head(getScanID(nc))
head(getSnpID(nc))
...
In addition to the three variables held in common with all NetCDF files (\Rcode{sampleID, chromosome} and \Rcode{position}),
...
NetCDF files, \Rcode{ncdfCreate}.  The two dimensions and three common variables will be created
...
We next call the \Rcode{ncdfAddData} function to populate the intensity NetCDF with the 
BAF and LRR values. The sample identifier \Rcode{scanID} is also populated at the same time.
...
\subsubsection*{Reading the BAF and LRR NetCDF file}
          
We also use the \Rcode{NcdfIntensityReader} class for BAF/LRR data.

<<>>=
(nc <- NcdfIntensityReader(bl.nc.file))
getBAlleleFreq(nc, snp=c(1,5), scan=c(1,3))
getLogRRatio(nc, snp=c(1,5), scan=c(1,3))
close(nc)
@ 
...
\Rcode{NcdfGenotypeReader}, \Rcode{SnpAnnotationDataFrame}, and \Rcode{ScanAnnotationDataFrame},
...
\Rcode{GenotypeData} objects, but take a \Rcode{NcdfIntensityReader}
object as the first argument.

<<>>=
# quality score, X and X intensity
qxyfile <- system.file("extdata", "illumina_qxy.nc", package="GWASdata")
nc <- NcdfIntensityReader(qxyfile)
qxyData <- IntensityData(NcdfIntensityReader(qxyfile), 
...
nc <- NcdfIntensityReader(blfile)
blData <- IntensityData(NcdfIntensityReader(blfile), 
                        snpAnnot=snpAnnot, scanAnnot=scanAnnot)
blData
...
chromIntensityPlot(blData, scanID[1], chrom[1], info=bincode[1], ideogram=FALSE)
@
...
estimation (MLE) of the IBD probabilities \Rcode{k1} and \Rcode{k0} 
...
and then graphs \Rcode{k1} versus \Rcode{k0}. The calculations are done
using the \Rpackage{gdsfmt} and \Rpackage{SNPRelate} packages developed as part 
of the GENEVA project.   \Rpackage{gdsfmt} is a library that uses a file
format similar to the NetCDF files, and \Rpackage{SNPRelate} performs the
IBD calculations using this data format.
                             

\subsubsection*{IBD Using Maximum Likelihood Estimation (MLE)}                             
      
We will convert the genotype NetCDF file to the \Rcode{gds} format with the function \Rcode{convertNcdfGds}.
...
The \Rcode{snpgdsIBDMLE} function gives square matrices with rows and columns equal to the number of
samples where entries are the \Rcode{k0} and \Rcode{k1} coefficient estimates, respectively, for each pairwise combination 
...
Then the function \Rcode{pedigreeClean} is called, which checks for basic pedigree errors such as not all mothers annotated as 
...
function call to \Rcode{pedigreeFindDuplicates}. We will also call \Rcode{pedigreeCheck} which determines if there are any 
...
\Rcode{one.person} is vector of family ids for one-person families. 
\Rcode{mismatch.sex} consists of family ids where sex of mother and/or father is incorrect.
\Rcode{impossible.related} consists of family ids where either child is mother of self or 
an individual is both child and mother of same person subfamilies. \Rcode{ident} is a matrix
with family id (families with subfamilies), subfamily identifier, individual ids of
persons in the subfamily. (In \Rcode{subfamilies.ident} the individual id's include 
individuals identified as mother or father who may not be in \Rcode{individ}.) 
...
among the samples by calling the function \Rcode{pedigreePairwiseRelatedness}. The relationships looked for as annotated are:
...
The orange line segments span the values for the expected \Rcode{k0}, \Rcode{k1} values, $\pm$2 standard deviations.
...
The function \Rcode{apartSnpSelection} randomly selects SNPs for which no pair of SNPs is closer than a user specified
...
We now use the function \Rcode{snpgdsIBDMoM} to perform IBD
...
and mother ids, where an entry of \Rcode{0} indicates no familial data.
As before, we use the function \Rcode{pedigreeClean} to check for
pedigree errors.
...
As with the Europeans, we use \Rcode{pedigreeFindDuplicates} to
    identify duplicates and and \Rcode{pedigreeCheck} to look for
singleton families, mothers/fathers whose sex does not match,
impossible relationships, or subfamilies.
...
Now from the verified sample list excluding duplicate samples, we can calculate the expected relationships
among the samples by calling the function \Rcode{pedigreePairwiseRelatedness}.
...
the function \Rcode{ncdfSetMissingGenotypes} to create a new NetCDF file
...
To run the Hardy-Weinberg test, we will filter out duplicates and non-founders. We will run \Rcode{gwasExactHW} for the samples with European continental ancestry only,
...
We will look at the values calculated from the function call to \Rcode{gwasExactHW}, which include p-values, minor allele frequency, and genotype counts for each SNP on each of the chromosome 
...
values. Finally, there are genotype counts for all chromosome types, which are stored as \Rcode{nAA, nAB} 
and \Rcode{nBB}.
...
To run the association test, we call the function
  \Rcode{assocTestRegression}.  (For survival analysis, the function
\Rcode{assocTestCPH} may be used.)
...
The cluster plots show that some of the SNPs have poor cluster plots, indicating genotyping artifacts.   In a real association study, the evidence from such SNPs would be discounted.
