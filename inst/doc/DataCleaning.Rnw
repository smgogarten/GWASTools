...
boxplot(scanAnnot$het.A[female] ~ scanAnnot$race[female], 
...
ped.complete$family[newfam] <- ped.complete$family[newfam] + 10000
...
As before, we use the function \Rcode{pedigreeCheck} to check for
pedigree errors.
...
As with the Europeans, we remove duplicates and fix subfamilies.
