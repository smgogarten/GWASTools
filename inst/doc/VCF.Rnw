...
This vignette demonstrates how to convert a Variant Call Format (VCF) file into a NetCDF
...
We use the \Rpackage{VariantAnnotation} package to read the VCF file.
<<>>=
...
alternate allele.
<<>>=
ref <- values(ref(vcf))[["REF"]]
alt <- values(alt(vcf))[["ALT"]]
# width of ref seq
rw <- width(ref)
# width of first alt seq
aw <- unlist(lapply(alt, function(x) {width(x[1])}))
# number of alternate genotypes
nalt <- elementLengths(alt)
...
REF <- as.character(values(ref(vcf))[["REF"]])
ALT <- as.character(unlist(values(alt(vcf))[["ALT"]]))
QUAL <- values(qual(vcf))[["QUAL"]]
FILTER <- values(filt(vcf))[["FILTER"]]
@
...
Create a genotype NetCDF file and add the integer sample IDs and genotypes.
<<>>=
ncfile <- "ex2.nc"
ncdfCreate(snp.annotation=snp.df, ncdf.filename=ncfile,
           n.samples=ncol(vcf), variables="genotype")
